...
#struct - Guardamos los codigos en hex
...
def createARPReply(IP,MAC):
    ...
    frame = ARPHeader + ARPReplyCode + bytes(myMAC) + struct.pack("!I", myIP) + MAC + struct.pack("!I", IP) + bytes(4)
...
def initARP(interface):
    ...
    if ARPResolution(myIP) != None:
        print("No inicializa el nivel ARP")
        return -1

    # Marcar a True la variable de nivel ARP
    arpInitialized = True

    return arpInitialized
